	...
	(function(rv){
		...
		function create(id,xml,callback){
			...
			if (oViewer.isHTML5possible()) {
				oViewer.embed();
				rv.xhttp.request(xml,function(o) {
		...
		function handleDeviceReady(e){
			logTime('deviceready');
		}

		function logTime(s,arguments){
			console.log('',s,rv.millis()-iTimeInit,arguments); // log
		}
